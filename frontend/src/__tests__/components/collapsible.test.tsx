...
describe('Collapsible', () => {
  it('제목을 렌더링한다', async () => {
    const { getByText } = await render(<Collapsible title="섹션"><Text>내용</Text></Collapsible>);
    expect(getByText('섹션')).toBeTruthy();
  });

  it('초기 상태에서 자식 요소가 숨겨져 있다', async () => {
    const { queryByText } = await render(<Collapsible title="섹션"><Text>숨겨진 내용</Text></Collapsible>);
    expect(queryByText('숨겨진 내용')).toBeNull();
  });

  it('제목을 누르면 자식 요소가 나타난다', async () => {
    const { getByText, queryByText } = await render(
      <Collapsible title="섹션"><Text>표시될 내용</Text></Collapsible>
    );
    await act(async () => {
  ...
  it('제목을 두 번 누르면 자식 요소가 다시 숨겨진다', async () => {
    const { getByText, queryByText } = await render(
      <Collapsible title="섹션"><Text>토글 내용</Text></Collapsible>
    );
    await act(async () => {
      fireEvent.press(getByText('섹션'));
    });
