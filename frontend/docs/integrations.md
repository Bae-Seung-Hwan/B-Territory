...
- [ ] `useLocation()`이 지도 화면에서 얻는 좌표를 `location:update`로 보내는 주기/쓰로틀링 결정 (훅 연결 자체는 완료)
...
- 원인: `use-registration-flow.ts`는 `createUserWithEmailAndPassword`로 Firebase 계정을 만든 뒤 `POST /api/auth/register`를 호출하는데, 백엔드 호출이 동기적으로 실패하면 방금 만든 Firebase 계정을 롤백(`user.delete()`)하지만, 그 사이 앱이 강제 종료되거나 네트워크가 끊기는 등 **중단**이 발생하면 이 롤백이 실행되지 않아 "유령 Firebase 계정"이 남는다.
- 현재 동작: 다음 로그인 시 `getMe()`가 `404` → 위에서 설명한 "이메일/비밀번호를 확인해달라"는 alert가 뜨고 세션도 정리된 채 로그인 화면에 머무름 (`login.tsx`의 `finishLogin`).
- 복구 경로(해결됨): `use-registration-flow.ts`에서 같은 이메일/비밀번호로 다시 가입을 시도하면 `createUserWithEmailAndPassword`가 `auth/email-already-in-use`로 실패하는데, 이때 같은 자격증명으로 `signInWithEmailAndPassword`를 시도한다. 성공하면 본인 계정이므로 계정을 새로 만들지 않고 `POST /api/auth/register`만 이어서 호출한다(비밀번호가 틀리면 남의 계정이라 여기서 실패하고 그대로 안내된다). 남아있는 세션에 기대지 않으므로 앱을 재시작한 뒤에도 복구된다. 이 경우는 이번 시도로 만든 계정이 아니므로 실패 시 롤백(`user.delete()`) 대상에서도 제외된다.
...
- [x] frontend에 Firebase SDK 설치 — `firebase`(JS SDK) + `@react-native-async-storage/async-storage`. 당시 Expo Go를 쓰고 있어 네이티브 모듈이 필요 없는 JS SDK를 골랐고, Dev Build로 전환한 지금도 그대로 쓰고 있다 ([decisions/0001-expo-go-vs-dev-build.md](./decisions/0001-expo-go-vs-dev-build.md) 참고)
...
- [ ] 구글 로그인 재활성화 — `login.tsx`의 "준비 중" 스텁(`handleGoogleLogin`)을 `useGoogleLogin` 실제 호출로 되돌리기. Dev Build 전환이 끝나 더 이상 막힌 상태가 아니다 (아래 ⚠️ Google 로그인 항목 참고)
- [x] "Firebase 계정은 있는데 백엔드 프로필 없음" 복구 — `use-registration-flow.ts`가 `auth/email-already-in-use`를 같은 자격증명 로그인으로 받아 "이어서 가입"으로 이어준다 (위 ⚠️ A 항목)
...
> **현재 상태:** 비활성화 사유였던 Expo Go 제약은 Dev Build 전환으로 해소됐지만,
> `login.tsx`의 "준비 중" 스텁은 아직 그대로다. 즉 아래 설명은 비활성화의 *배경*이고,
> 지금은 되돌릴 수 있는 상태인데 아직 되돌리지 않은 것이다.
...
- **재활성화 방법**: 네이티브 `@react-native-google-signin/google-signin`으로 교체하면서 `login.tsx`의 스텁을 실제 훅 호출로 되돌린다. Dev Build 전환은 이미 끝났으므로 지금 착수 가능하다 ([decisions/0001-expo-go-vs-dev-build.md](./decisions/0001-expo-go-vs-dev-build.md) 참고)
...
갖춰졌다([decisions/0001-expo-go-vs-dev-build.md](./decisions/0001-expo-go-vs-dev-build.md)).
다만 `eas.json`에는 아직 android 프로필만 있어 iOS 빌드를 한 번도 돌리지 않았으므로,
아래 작업은 iOS 빌드 파이프라인을 세우는 시점에 함께 진행한다.

### 필요 작업 (TODO)

- [x] `app.config.js`에 `ios.bundleIdentifier` 설정 (Dev Build 전환의 일부)
