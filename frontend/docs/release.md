...
소개 페이지의 내려받기 버튼은 `scripts/build-landing-page.mjs`의 `STORE_URL`로 간다. 원스토어 주소는 이미 들어가 있으니 **주소가 바뀌거나 스토어가 늘면 그 값을 고친다**(compliance.md 3.2절). 비우면 버튼이 링크가 아니라 "스토어 등록 준비 중" 표시가 된다.
