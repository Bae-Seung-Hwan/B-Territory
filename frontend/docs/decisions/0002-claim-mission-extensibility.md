...
> ⚠️ **이름이 겹치는 다른 기능이 있다.** 이 문서의 "미션"은 `POST /api/claims/visit`으로 지점을
> 점령하는 *점령 미션*(`constants/claimMissions.ts`)이다. 나중에 추가된 *관광지 미션*
> (`api/missions.ts` — 체크인 후 별점·후기 작성)은 별개 기능이고 **이 확장 구조를 쓰지 않는다.**
> 관광지 미션을 늘릴 때 아래 표를 따라가면 엉뚱한 곳을 고치게 된다.
