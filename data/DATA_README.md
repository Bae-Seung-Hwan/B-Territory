...
| `sigungu_code` 결측(빈 값) 수 | 0 |
| 주소↔`sigungu_code` 불일치(원본 오류) | 3건 (아래) |

> **주소↔`sigungu_code` 불일치 (원본 데이터 오류, 코드로 자동 정정 불가)**: 아래 3건은 `sigungu_code` 값 자체가 유효한 숫자 코드라 시딩 정규화(`normalizeSigunguCode`)를 통과하지만, 주소의 구/군과 어긋나 해당 장소의 점령이 엉뚱한 구로 집계되거나 두 장소가 한 행에 병합돼 있습니다. 시딩 스크립트가 주소와 코드를 교차검증해 **경고**로 출력하니(`seed:spots` 로그 확인), CSV 갱신 시 함께 점검하세요.
>
> | mission_id | 내용 |
> |---|---|
