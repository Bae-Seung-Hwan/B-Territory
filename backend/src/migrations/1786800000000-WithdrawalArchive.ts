 ...
 *
 * 남기는 것은 계정 식별자(이메일)·약관 동의·신고 연결 셋뿐이다. 이 범위와 6개월이라는
 * 보존기간은 개인정보처리방침 제3조 3항이 정한 것이므로 임의로 넓히지 않는다. 만료분은
 * `purge` 잡이 지우며, 그때 `consent_archives`는 FK CASCADE로,
 * `reports.withdrawnTargetId`는 SET NULL로 함께 정리된다.
 */
export class WithdrawalArchive1786800000000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TABLE "withdrawn_accounts" ("id" SERIAL NOT NULL, "email" character varying NOT NULL, "withdrawnAt" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(), CONSTRAINT "PK_withdrawn_accounts" PRIMARY KEY ("id"))`,
    );
    await queryRunner.query(
      `CREATE INDEX "IDX_withdrawn_accounts_email" ON "withdrawn_accounts" ("email")`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `DROP INDEX "public"."IDX_withdrawn_accounts_email"`,
    );
